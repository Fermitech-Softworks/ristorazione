...
from ristorazione.dependencies import get_auth_token, get_db
from ristorazione.authentication import get_current_user, check_admin
from ristorazione.database.crud import get_users, create_user, get_user, update_user
from sqlalchemy.orm import Session
from ristorazione.database import schemas, models
...
async def update_user_(user: schemas.UserCreatePlain, user_id: int, db: Session = Depends(get_db),
    ...
    Allows user update
    """
    if user_id != current_user.uid and check_admin(current_user):
